...
int XMain()
{
	while(XPending(dpy) > 0) {
		...
		switch (event.type) {
			...
			case KeyPress: {
				...
				if((XLookupString((XKeyEvent *)&event,buffer,1,&keysym,NULL) == 1) && (keysym == (KeySym)XK_Escape)) Program.IsRunning = false;
				Program.Key[keysym & 0xFF] = true;
				break; }
			case KeyRelease: {
				KeySym     keysym;
				char       buffer[1];
				XLookupString((XKeyEvent *)&event,buffer,1,&keysym,NULL);
				Program.Key[keysym & 0xFF] = false;
				break; }

			case ButtonPress: {
				switch(event.xbutton.button) {
					case 1:
						Program.MouseCenterX = event.xbutton.x;
						Program.MouseCenterY = event.xbutton.y;
						break;
				}
				break; }
			...
			case MotionNotify: {
				if(event.xmotion.state & Button1Mask) {
					Program.MousePosX = event.xbutton.x;
					Program.MousePosY = event.xbutton.y;
	...
	if(Program.Key[XK_w]) ca_Movement(false, 6.0f);
	if(Program.Key[XK_s]) ca_Movement(false, -6.0f);
	if(Program.Key[XK_a]) ca_Movement(true, -6.0f);
	if(Program.Key[XK_d]) ca_Movement(true, 6.0f);

	if(Program.Key[XK_1] && zOptions.SceneNo > 0) {
		Viewer.LevelID--;
		sv_Init();
		Program.Key[VK_F1] = false;
	}

	if(Program.Key[XK_2] && zOptions.SceneNo < zGame.SceneCount) {
		Viewer.LevelID++;
		sv_Init();
