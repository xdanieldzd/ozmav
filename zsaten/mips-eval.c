...
int mips_ReportFunc(unsigned int target)
{
	...
	if(func_no < 0){
		...
		for(i=0;i<4;i++){
			...
			if((seg == 6 || seg == 4 || seg == 5) && !(regs[i+MIPS_REG_A0] & 0x00F00003) ){
	...
	dbgprintf(1, MSK_COLORTYPE_WARNING, " - Recording %08X with args 0x%08X, 0x%08X, 0x%08X, 0x%08X\n", target,
...
void * mips_GetFuncArg(unsigned int target, int argc, int nonzero)
{
	...
	target &= 0x0FFFFFFF;
	dbgprintf(1, MSK_COLORTYPE_WARNING, "  - Requesting %08X #%i\n", target, argc);
	...
	/* was it found ? */
	if(func_no < 0){
		dbgprintf(1, MSK_COLORTYPE_WARNING, "  - %08X not caught\n", target);
		return NULL;
	}

	ret = &mips_funcs_found[func_no].args[argc];
	dbgprintf(1, MSK_COLORTYPE_WARNING, "  - Reporting %08X %i : 0x%08X\n", target, argc, *ret);
	return ret;
}
