...
void md_InitModelDumping(int SceneNo)
{
	...
	sprintf(FilePath, "dump//scene_%i.obj", (unsigned int)SceneNo);
	zProgram.FileWavefrontObj = fopen(FilePath, "w");

	sprintf(MtlFilename, "scene_%i.mtl", (unsigned int)SceneNo);
	sprintf(FilePath, "dump//%s", MtlFilename);
...
void md_CreateMaterial(int TextureID, unsigned char * TextureData)
{
	...
	sprintf(TextureFilename, "scene_%i_fmt0x%02X_0x%08X.png", (unsigned int)zOptions.SceneNo, zTexture[TextureID].Format, zTexture[TextureID].Offset);

	char TexturePath[256];
	sprintf(TexturePath, "dump//%s", TextureFilename);
